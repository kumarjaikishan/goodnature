...
import { setloader, setpromotor } from '../../store/login';


const Promotor = ({  notification }) => {
  ...
  const sub = async () => {
    let { senior, name, id, contact, address } = inp;
    // alert(name);
    if (!name || !id || !contact || !address || !senior) {
      ...
      try {
        const result = await fetch('http://localhost:5000/addpromoter', {
          method: "POST",
          headers: {
            "Content-Type": "application/json"
          },
          body: JSON.stringify({
            senior, name, id, contact, address
          })
        })
  ...
  const fetching = async () => {
    ...
    if (!userid) {
      ...
      const result = await fetch('http://localhost:5000/promotorlist', {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
        },
  ...
  const currentpost = log.promotor[0].slice(firstpostindex, lastpostindex);
